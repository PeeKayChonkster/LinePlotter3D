...
var points = [Vector3(0,0,0),Vector3(0,5,0)]
...
onready var startThickness = thickness
onready var endThickness = thickness
...
func _process(_delta):
	...
	for i in range(points.size() - 1):
		...
		var orthogonalABEnd = (cameraOrigin - ((A + B) / 2)).cross(AB).normalized() * thickness;
		...
		if i == points.size() - 2:
			...
			if drawCorners:
				...
				var BC = C - B;
				var orthogonalBCStart = (cameraOrigin - ((B + C) / 2)).cross(BC).normalized() * thickness;
